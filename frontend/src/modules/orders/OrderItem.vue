...
const orderPrice = computed(() => {
	let price = 0;
	props.order.orderPizzas.forEach((pizza) => (price += pizza.quantity * pizza.price));
	props.order.orderMisc.forEach((misc) => (price += misc.quantity * misc.price));
	return price;
});
