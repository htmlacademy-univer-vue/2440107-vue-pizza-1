...
import { miscsHandler } from "@/common/helpers";
...
export const useCartStore = defineStore("cart", {
	...
	getters: {
		totalPrice: (state) => {
			const pizzasPrice = state.pizzas.length
				? state.pizzas.reduce((sum, pizza) => sum + pizza.price * pizza.quantity, 0)
				: 0;
	...
	actions: {
		...
		updateMisc(newMisc) {
			const targetIndex = this.miscs.findIndex((misc) => misc.id === newMisc.id);

			this.miscs.splice(targetIndex, 1, newMisc);
		},
		updatePizza(newPizza) {
			const targetIndex = this.pizzas.findIndex((pizza) => pizza.id === newPizza.id);
