          ...
          return {
              ...
              ? order.orderPizzas.map((pizza) => {
                  const ingredients = state.ingredients.map((ingredient) => {
                    const targetIngredient = pizza.ingredients.find(
                      (orderedIngr) => orderedIngr.ingredientId === ingredient.id
                    );
                    return {
                      ...ingredient,
                      quantity: targetIngredient ? targetIngredient.quantity : 0
                    };
                  });

                  const dough = state.dough.find((dough) => dough.id === pizza.doughId);
                  const size = state.sizes.find((size) => size.id === pizza.sizeId);
                  const sauce = state.sauces.find((sauce) => sauce.id === pizza.sauceId);
            ...
            orderMisc: order.orderMisc
              ? order.orderMisc.map((orderedMisc) => {
                  const misc = cartStore.miscs.find((misc) => misc.id === orderedMisc.miscId);
