...
import { getPizzaPrice } from '@helpers';
...
const dropped = ({ ingredient, index }) => {
  ...
  if (target.name) {
    target.quantity = target.quantity === 3 ? target.quantity : ++target.quantity;
  } else {
